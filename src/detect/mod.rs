...
use crate::{
    types::{ConnectionSecurity, Error, ErrorKind, Result},
...
impl DetectClient {
    ...
    pub async fn from_email(email_address: &str) -> Result<Config> {
        ...
        let mut split = email_address.split(Self::AT_SYMBOL);
        ...
        if config.is_none() {
            ...
            let results = Self::check_sockets(sockets_to_check).await?;
...
mod test {
    ...
    async fn from_email() {
        ...
        let config = super::DetectClient::from_email(email).await.unwrap();
